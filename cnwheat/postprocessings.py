...
def generate_graphs(axes_df=None, hiddenzones_df=None, organs_df=None, elements_df=None, soils_df=None, graphs_dirpath='.'):
    ...
    # Obsolete variables? 
    # 'PARa': u'Absorbed PAR (�mol m$^{-2}$ s$^{-1}$)',
    # 'An': u'Net photosynthesis (�mol m$^{-2}$ s$^{-1}$)', 
    # 'Rd': u'Mitochondrial respiration rate of organ in light (�mol C h$^{-1}$)', 
    # 'gs': u'Conductance stomatique (mol m$^{-2}$ s$^{-1}$)',
    graph_variables_ph_elements = {'Ag': u'Gross photosynthesis (�mol m$^{-2}$ s$^{-1}$)', 'Tr':u'Organ surfacic transpiration rate (mmol H$_{2}$0 m$^{-2}$ s$^{-1}$)', 'Transpiration':u'Organ transpiration rate (mmol H$_{2}$0 s$^{-1}$)', 'Ts': u'Temperature surface (�C)', 
    ...
    # 4) Hidden zones
    # Obsolete variables? 
    # 'Respi_growth': u'Growth respiration (�mol C)',
    # 'delta_leaf_L':u'Delta leaf length (m)',
    # 'leaf_L': u'Leaf length (m)',
    # 'leaf_dist_to_emerge': u'Length for leaf emergence (m)',
    # 'sucrose_consumption_mstruct': u'Consumption of sucrose for growth (�mol C)' 
    graph_variables_hiddenzones = {'Conc_Sucrose':u'[Sucrose] (�mol g$^{-1}$ mstruct)', 'Conc_Amino_Acids':u'[Amino Acids] (�mol g$^{-1}$ mstruct)', 'Conc_Proteins': u'[Proteins] (g g$^{-1}$ mstruct)', 'Conc_Fructan':u'[Fructan] (�mol g$^{-1}$ mstruct)',
                                    'Unloading_Sucrose':u'Sucrose unloading (�mol C)', 'Unloading_Amino_Acids':u'Amino_acids unloading (�mol N)', 'mstruct': u'Structural mass (g)'}
