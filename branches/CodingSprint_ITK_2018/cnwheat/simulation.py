...
class Simulation(object):
    ...
    def initialize(self, population, soils):
        ...
        for soil in soils.itervalues():
            ...
            soil.initialize(delta_t=self.delta_t)
                ...
                for phytomer in axis.phytomers:
                    ...
                    for organ in (phytomer.chaff, phytomer.peduncle, phytomer.lamina, phytomer.internode, phytomer.sheath, phytomer.hiddenzone):
                        ...
                        for element in (organ.exposed_element, organ.enclosed_element):
                            if element is None:
                                continue
                            element.initialize(delta_t=self.delta_t)
        ...
        **t** is an argument passed to :func:`odeint(func, y0, t, args=(),...) <scipy.integrate.odeint>`. `y` is
        ...
        for plant in self.population.plants:
            for axis in plant.axes:
                ...
                roots_regul_transpiration = axis.roots.calculate_regul_transpiration(total_surfacic_transpiration)

                # compute the flows from/to the roots to/from photosynthetic organs
                axis.roots.Uptake_Nitrates, roots_HATS_LATS = axis.roots.calculate_Uptake_Nitrates(soil.Conc_Nitrates_Soil, axis.roots.nitrates, axis.roots.sucrose, self.delta_t)
                soil_contributors.append((axis.roots.Uptake_Nitrates, plant.index)) # TODO: TEMP!!!
                roots_R_Nnit_upt = self.respiration_model.RespirationModel.R_Nnit_upt(axis.roots.Uptake_Nitrates, axis.roots.sucrose)
                roots_Export_Nitrates = axis.roots.calculate_Export_Nitrates(axis.roots.nitrates, roots_regul_transpiration, self.delta_t)
                roots_Export_Amino_Acids = axis.roots.calculate_Export_Amino_Acids(axis.roots.amino_acids, roots_regul_transpiration, self.delta_t)
                roots_Export_cytokinins = axis.roots.calculate_Export_cytokinins(axis.roots.cytokinins, roots_regul_transpiration, self.delta_t)
                ...
                for phytomer in axis.phytomers:
                    ...
                    for organ in (phytomer.chaff, phytomer.peduncle, phytomer.lamina, phytomer.internode, phytomer.sheath):
                        ...
                        for element in (organ.exposed_element, organ.enclosed_element):
                            ...
                            element_Photosynthesis = element.calculate_total_Photosynthesis(element.Ag, element.green_area, self.delta_t)
                            ...
                            element_S_Starch = element.calculate_S_Starch(element.triosesP, self.delta_t)
                            element_D_Starch = element.calculate_D_Starch(element.starch, self.delta_t)
                            element_S_Sucrose = element.calculate_S_Sucrose(element.triosesP, self.delta_t)
                            element_R_phloem_loading, element.Loading_Sucrose = self.respiration_model.RespirationModel.R_phloem(element.Loading_Sucrose, element.sucrose, element.mstruct*element.__class__.PARAMETERS.ALPHA)
                            element_Regul_S_Fructan = element.calculate_Regul_S_Fructan(element.Loading_Sucrose, self.delta_t)
                            element_D_Fructan = element.calculate_D_Fructan(element.sucrose, element.fructan, self.delta_t)
                            element_S_Fructan = element.calculate_S_Fructan(element.sucrose, element_Regul_S_Fructan, self.delta_t)
                            element_Nitrates_import = element.calculate_Nitrates_import(roots_Export_Nitrates, element.Transpiration, axis.Total_Transpiration)
                            element_Amino_Acids_import = element.calculate_Amino_Acids_import(roots_Export_Amino_Acids, element.Transpiration, axis.Total_Transpiration)
                            element_S_Amino_Acids = element.calculate_S_amino_acids(element.nitrates, element.triosesP, self.delta_t)
                            element_R_Nnit_red, element_S_Amino_Acids = self.respiration_model.RespirationModel.R_Nnit_red(element_S_Amino_Acids, element.sucrose, element.mstruct*element.__class__.PARAMETERS.ALPHA)
                            element_S_Proteins = element.calculate_S_proteins(element.amino_acids, self.delta_t)
                            element_k_proteins, element_D_Proteins = element.calculate_D_Proteins(element.proteins, element.cytokinins, self.delta_t)
                            element_cytokinins_import = element.calculate_cytokinins_import(roots_Export_cytokinins, element.Transpiration, axis.Total_Transpiration)
                            element_D_cytokinins = element.calculate_D_cytokinins(element.cytokinins)

                            # compartments derivatives
                            starch_derivative = element.calculate_starch_derivative(element_S_Starch, element_D_Starch)
                            element_R_residual,_ = self.respiration_model.RespirationModel.R_residual(element.sucrose, element.mstruct*element.__class__.PARAMETERS.ALPHA, element.Total_Organic_Nitrogen, self.delta_t, element.Ts)
                            element_sum_respi = element_R_phloem_loading + element_R_Nnit_red + element_R_residual
                            sucrose_derivative = element.calculate_sucrose_derivative(element_S_Sucrose, element_D_Starch, element.Loading_Sucrose, element_S_Fructan, element_D_Fructan, element_sum_respi)
                            triosesP_derivative = element.calculate_triosesP_derivative(element_Photosynthesis, element_S_Sucrose, element_S_Starch, element_S_Amino_Acids)
                            fructan_derivative = element.calculate_fructan_derivative(element_S_Fructan, element_D_Fructan)
                            nitrates_derivative = element.calculate_nitrates_derivative(element_Nitrates_import, element_S_Amino_Acids)
                            amino_acids_derivative = element.calculate_amino_acids_derivative(element_Amino_Acids_import, element_S_Amino_Acids, element_S_Proteins, element_D_Proteins, element.Loading_Amino_Acids)
                            proteins_derivative = element.calculate_proteins_derivative(element_S_Proteins, element_D_Proteins)
                            cytokinins_derivative = element.calculate_cytokinins_derivative(element_cytokinins_import, element_D_cytokinins)
                ...
                if axis.grains is not None:
                    ...
                    # intermediate variables
                    grains_RGR_Structure = axis.grains.calculate_RGR_Structure(axis.phloem.sucrose, axis.mstruct)
                    grains_structural_dry_mass = axis.grains.calculate_structural_dry_mass(axis.grains.structure)

                    # flows
                    grains_S_grain_structure = axis.grains.calculate_S_grain_structure(axis.grains.structure, grains_RGR_Structure, self.delta_t)
                    grains_S_grain_starch = axis.grains.calculate_S_grain_starch(axis.phloem.sucrose, axis.mstruct, self.delta_t)
                    grains_S_Proteins = axis.grains.calculate_S_proteins(grains_S_grain_structure, grains_S_grain_starch, axis.phloem.amino_acids, axis.phloem.sucrose, grains_structural_dry_mass)
                    # compartments derivatives
                    grains_R_grain_growth_struct, grains_R_grain_growth_starch = self.respiration_model.RespirationModel.R_grain_growth(grains_S_grain_structure, grains_S_grain_starch, grains_structural_dry_mass)
                    structure_derivative = axis.grains.calculate_structure_derivative(grains_S_grain_structure, grains_R_grain_growth_struct)
                    starch_derivative = axis.grains.calculate_starch_derivative(grains_S_grain_starch, grains_structural_dry_mass, grains_R_grain_growth_starch)
                    proteins_derivative = axis.grains.calculate_proteins_derivative(grains_S_Proteins)
                ...
                roots_S_Amino_Acids = axis.roots.calculate_S_amino_acids(axis.roots.nitrates, axis.roots.sucrose, self.delta_t)
                roots_R_Nnit_red, roots_S_Amino_Acids = self.respiration_model.RespirationModel.R_Nnit_red(roots_S_Amino_Acids, axis.roots.sucrose, axis.roots.mstruct*model.Roots.PARAMETERS.ALPHA, root=True)
                roots_C_exudation, roots_N_exudation = axis.roots.calculate_exudation(axis.roots.Unloading_Sucrose, axis.roots.sucrose, axis.phloem.sucrose, axis.roots.amino_acids, axis.phloem.amino_acids)
                roots_S_cytokinins = axis.roots.calculate_S_cytokinins(axis.roots.sucrose, axis.roots.nitrates, self.delta_t)

                # compartments derivatives
                roots_R_residual,_ = self.respiration_model.RespirationModel.R_residual(axis.roots.sucrose, axis.roots.mstruct*model.Roots.PARAMETERS.ALPHA, axis.roots.Total_Organic_Nitrogen, self.delta_t, soil.Tsoil)
                roots_sum_respi = roots_R_Nnit_upt + roots_R_Nnit_red + roots_R_residual
                sucrose_derivative = axis.roots.calculate_sucrose_derivative(axis.roots.Unloading_Sucrose, roots_S_Amino_Acids, roots_C_exudation, roots_sum_respi)
                nitrates_derivative = axis.roots.calculate_nitrates_derivative(axis.roots.Uptake_Nitrates, roots_Export_Nitrates, roots_S_Amino_Acids)
                amino_acids_derivative = axis.roots.calculate_amino_acids_derivative(axis.roots.Unloading_Amino_Acids, roots_S_Amino_Acids, roots_Export_Amino_Acids, roots_N_exudation)
                cytokinins_derivative = axis.roots.calculate_cytokinins_derivative(roots_S_cytokinins, roots_Export_cytokinins)
    ...
    def _update_model(self, compartments_values):
        ...
        for plant in self.population.plants:
            for axis in plant.axes:
                ...
                axis.roots.Export_Nitrates = axis.roots.calculate_Export_Nitrates(axis.roots.nitrates, axis.roots.regul_transpiration, self.delta_t)
                axis.roots.Export_Amino_Acids = axis.roots.calculate_Export_Amino_Acids(axis.roots.amino_acids, axis.roots.regul_transpiration, self.delta_t)
                axis.roots.Export_cytokinins = axis.roots.calculate_Export_cytokinins(axis.roots.cytokinins, axis.roots.regul_transpiration, self.delta_t)
                ...
                for phytomer in axis.phytomers:
                    ...
                    if phytomer.hiddenzone is not None:
                        ...
                        # Fructan synthesis
                        Regul_Sfructanes = hiddenzone.calculate_Regul_S_Fructan(hiddenzone.Unloading_Sucrose, self.delta_t)
                        hiddenzone.S_Fructan = hiddenzone.calculate_S_Fructan(hiddenzone.sucrose, Regul_Sfructanes, self.delta_t)
                    ...
                    for organ in (phytomer.chaff, phytomer.peduncle, phytomer.lamina, phytomer.internode, phytomer.sheath):
                        ...
                        for element in (organ.exposed_element, organ.enclosed_element):
                            ...
                            # flows
                            if element.is_growing: #: Export of sucrose and amino acids towards the hidden zone
                                element.Loading_Sucrose = element.calculate_export_sucrose(element.sucrose, hiddenzone.sucrose, hiddenzone.mstruct, self.delta_t)
                                element.Loading_Amino_Acids = element.calculate_Export_Amino_Acids(element.amino_acids, hiddenzone.amino_acids, hiddenzone.mstruct, self.delta_t)
                            else: #: Loading of sucrose and amino acids towards the phloem
                                phloem_contributors.append(element)
                                element.Loading_Sucrose = element.calculate_Loading_Sucrose(element.sucrose, axis.phloem.sucrose, axis.mstruct, self.delta_t)
                                element.Loading_Amino_Acids = element.calculate_Loading_Amino_Acids(element.amino_acids, axis.phloem.amino_acids, axis.mstruct, self.delta_t)

                            element.S_Starch = element.calculate_S_Starch(element.triosesP, self.delta_t)
                            element.D_Starch = element.calculate_D_Starch(element.starch, self.delta_t)
                            element.S_Sucrose = element.calculate_S_Sucrose(element.triosesP, self.delta_t)
                            element.R_phloem_loading, element.Loading_Sucrose = self.respiration_model.RespirationModel.R_phloem(element.Loading_Sucrose, element.sucrose, element.mstruct*element.__class__.PARAMETERS.ALPHA)
                            element.Regul_S_Fructan = element.calculate_Regul_S_Fructan(element.Loading_Sucrose, self.delta_t)
                            element.D_Fructan = element.calculate_D_Fructan(element.sucrose, element.fructan, self.delta_t)
                            element.S_Fructan = element.calculate_S_Fructan(element.sucrose, element.Regul_S_Fructan, self.delta_t)
                            ...
                            element.S_Amino_Acids = element.calculate_S_amino_acids(element.nitrates, element.triosesP, self.delta_t)
                            ...
                            element.S_Proteins = element.calculate_S_proteins(element.amino_acids, self.delta_t)
                            element.k_proteins, element.D_Proteins = element.calculate_D_Proteins(element.proteins, element.cytokinins, self.delta_t)
                ...
                if axis.grains is not None:
                    ...
                    # flows
                    axis.grains.S_grain_structure = axis.grains.calculate_S_grain_structure(axis.grains.structure, axis.grains.RGR_Structure, self.delta_t)
                ...
                axis.roots.C_exudation, axis.roots.N_exudation = axis.roots.calculate_exudation(axis.roots.Unloading_Sucrose, axis.roots.sucrose, axis.phloem.sucrose, axis.roots.amino_acids, axis.phloem.amino_acids)
                axis.roots.S_cytokinins = axis.roots.calculate_S_cytokinins(axis.roots.sucrose, axis.roots.nitrates, self.delta_t)

                axis.roots.R_residual,_ = self.respiration_model.RespirationModel.R_residual(axis.roots.sucrose, axis.roots.mstruct*model.Roots.PARAMETERS.ALPHA, axis.roots.Total_Organic_Nitrogen, self.delta_t, soil.Tsoil)
